...
function Execute-Xunit($projectName) {
	...
	if ($buildTools.UseMsBuild) {
		Execute-Xunit-MsBuild $projectName $artifactsPath
	}
    elseif ($buildTools.UseDotNetCli){
...
function Pack-DotNetCli($project, $version) {		
	...
	exec { dotnet pack $projectFilePath --configuration $config --no-build --no-restore --output ..\$artifactsPath -p:BuildVersion=$version }  
}
